...
def train(epoch, model, feeder, input_format, save_path="./", lr_decay=1.0, test=False):
    ...
    logger.info("train: epoch (%i), size (%i), iteration in an epoch (%i), batch size(%i)" % (epoch, feeder.n, feeder.iterator_length, feeder.batch_size))
...
def get_options(parser):
    ...
    parser.add_argument('model', help='Name of model to use. (default: cnn_char)',  default='cnn_char', type=str, **share_param)
    parser.add_argument('-e', '--epoch', help='Epoch number. (default: 500)',  default=50, type=int, **share_param)
    parser.add_argument('-b', '--batch', help='Batch size. (default: 100)', default=100, type=int, **share_param)
    parser.add_argument('-l', '--lr', help='Learning rate. (default: 0.0001)', default=0.0001, type=float, **share_param)
    parser.add_argument('-c', '--clip', help='Gradient clipping. (default: None)', default=None, type=float, **share_param)
    parser.add_argument('-k', '--keep', help='Keep rate for Dropout. (default: 1.0)', default=1.0, type=float, **share_param)
    parser.add_argument('-n', '--norm', help='Decay for batch normalization. if batch is 100, 0.95 (default: None)', default=None, type=float, **share_param)
    parser.add_argument('-d', '--decay_lr', help='Decay index for learning rate (default: 1.0)',  default=1.0, type=float, **share_param)
...
if __name__ == '__main__':
    ...
    # load model
    model_instance = sequence_modeling.get_model_instance(model_name=args.model, embedding_model=embedding_model, learning_rate=args.lr, gradient_clip=args.clip, batch_norm=args.norm, keep_prob=args.keep)

    # train
    data_feeder = sequence_modeling.BatchFeeder(data["sentence"], data["label"], batch_size=args.batch,validation=0.05, process=model_instance["processing"])
    train(model=model_instance["model"], input_format=model_instance["input_format"], epoch=args.epoch, feeder=data_feeder, save_path=path, lr_decay=args.decay_lr, test=False)
